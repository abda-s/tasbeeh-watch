...
static std::string build_state(uint32_t edge_since, uint32_t log_since) {
    char b[512];
    std::string j = "{";
...
static void handle(int fd, const std::string &req) {
    ...
    if (path == "/" || path == "/index.html") {
        ...
        respond(fd, 200, "application/json", build_state(atoi(q["edges"].c_str()), atoi(q["log"].c_str())));
    } else if (path == "/frame") {
        ...
        for (const char *k : {"fire", "buzz", "reboot", "factory"})
            if (q.count(k)) pending.push_back(std::string(k) + "=" + q[k]);
        respond(fd, 200, "application/json", "{\"ok\":1}");
    } else {
